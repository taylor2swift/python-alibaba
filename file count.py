...
import jieba
import nltk
#字典映射，实现switch/case功能，根据文件夹路径，给文章贴相应的标签
    ...
    for s in pathDir:
        ...
        if os.path.isfile(newDir) :         #如果是文件
            ...
            if os.path.splitext(newDir)[1] == ".txt" or os.path.splitext(newDir)[1] == ".TXT":  # 判断是否是txt
                with open(newDir, 'r',errors='ignore') as f:  # 打开文件    # txt是用ANSI编码格式的 r代表read 以只读模式打开?
                    s = f.read()
                    p = s.find("【 正  文 】")
                    if p > -1:
                        p += len("【 正  文 】")
                ...
                a = ([x for x in jieba.cut(s) if len(x) >= 2])
                word_str = " ".join(a)
...
eachFile("C:/Users/岑兮归无意/Documents/Tencent Files/2684581045/FileRecv/文本分类语料库/文本分类语料库")   #/交通214
print( words_list)
